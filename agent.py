...
class DebateCoordinator:
    def __init__(self, agents: List[DebateAgent], rounds_before_promote: int = 3):
        ...
        # Define the state graph for the debate
        self.workflow = StateGraph(state_schema=MessagesState)
        self.workflow.add_node("proposition", self.agents[0].generate_response)
        self.workflow.add_node("opposition", self.agents[1].generate_response)
        self.workflow.add_edge(START, "proposition")
        self.workflow.add_edge("proposition", "opposition")
        self.workflow.add_edge("opposition", "proposition")
        self.app = self.workflow.compile(checkpointer=MemorySaver())

    async def conduct_round(self, topic: str, proposition: str, opposition: str, config: Dict):
        """Conduct a debate round using the state graph."""
        print(f"\n=== 第 {self.round_counter + 1} 轮辩论 ===")

        # Proposition agent's turn
        prop_response, prop_feedback = await self.app.ainvoke(
            {"messages": [HumanMessage(content=f"作为正方，请回应：{proposition}")]},
            config=config
        )
        print(f"\n正方：{prop_response['messages'][-1].content}")

        # Opposition agent's turn
        oppo_response, oppo_feedback = await self.app.ainvoke(
            {"messages": [HumanMessage(content=f"作为反方，请回应：{opposition}。对方刚才说：{prop_response['messages'][-1].content}")]},
            config=config
        )
        print(f"\n反方：{oppo_response['messages'][-1].content}")

        # Log the debate round
        self.debate_log.append({
            "round": self.round_counter + 1,
            "responses": {
                "proposition": (prop_response["messages"][-1].content, prop_feedback),
                "opposition": (oppo_response["messages"][-1].content, oppo_feedback)
            }
        })
